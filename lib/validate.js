...
const itemInputSchema = z.object({
  title: z.string().min(1, 'Title is required and must be non-empty').max(200, 'Title max 200 chars').trim(),
  section: z.string().min(1, 'Section is required').max(50, 'Section max 50 chars').trim(),
  category: z.string().min(1, 'Category is required').max(50, 'Category max 50 chars').trim(),
  ...
  combatPoints: z.string().max(20, 'Command Points max 20 chars').trim().optional(),
...
const categoryInputSchema = z.object({
  label: z.string().min(1, 'Label is required').max(100, 'Label max 100 chars').trim(),
  id: z.string().max(50, 'ID max 50 chars').trim().optional(),
  section: z.string().max(50, 'Section max 50 chars').trim().optional(),
  parentId: z.string().max(50, 'Parent ID max 50 chars').optional(),
