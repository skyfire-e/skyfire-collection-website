...
// Rename (label only): section without id renames the section itself;
// id renames a top-level category/group; id + parentId renames a nested leaf.
// IDs never change here — items reference categories by id, URLs stay stable.
const categoryRenameSchema = z.object({
  section: z.string().trim().min(1, 'Section is required').max(50, 'Section max 50 chars'),
  id: z.string().trim().min(1).max(50, 'ID max 50 chars')
    .regex(/^[A-Za-z0-9-]+$/, 'Invalid category ID format').optional(),
  parentId: z.string().trim().min(1).max(50, 'Parent ID max 50 chars')
    .regex(/^[A-Za-z0-9-]+$/, 'Invalid parent ID format').optional(),
  label: z.string().trim().min(1, 'Label is required').max(100, 'Label max 100 chars')
});
